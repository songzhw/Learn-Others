package ca.six.ui.others.rv.firstex;
...
public class FirstExRecyclerView extends RecyclerView {
    private int defaultItemHeight, featuredItemHeight, maxDistance, diffHeight, itemToResize;
    private int totalItemsInView = 0;
    private FeatureRecyclerViewAdapter adapter;

    public FirstExRecyclerView(Context context) {
        super(context);
        init(context, null);
    }

    public FirstExRecyclerView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    private void init(Context context, AttributeSet attrs) {
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.FirstExRecyclerView);
    ...
    private void initAttributes(TypedArray array) {
        defaultItemHeight = (int) array.getDimension(R.styleable.FirstExRecyclerView_defaultItemHeight,
                getResources().getDimension(R.dimen.defaultItemHeight));
        featuredItemHeight = (int) array.getDimension(R.styleable.FirstExRecyclerView_featuredItemHeight,
