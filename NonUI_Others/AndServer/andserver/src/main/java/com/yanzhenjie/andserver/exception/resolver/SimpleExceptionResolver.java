...
public class SimpleExceptionResolver implements ExceptionResolver {
    ...
    public final void resolveException(Exception e, HttpRequest request, HttpResponse response, HttpContext context) {
        RespStub respStub = resolveException(e, request, response);
        response.setStatusCode(respStub.getHttpCode());
        response.setEntity(respStub.getHttpEntity());
        response.setHeaders(respStub.getHeaders());
    }

    public RespStub resolveException(Exception e, HttpRequest request, HttpResponse response) {
        return resolveException(e);
    }

    protected RespStub resolveException(Exception e) {
        if (e instanceof BaseException) {
            BaseException exception = (BaseException) e;
            return new RespStub(exception.getHttpCode(), exception.getHttpBody());
        }
        String message = String.format("Server error occurred:\n%1$s", e.getMessage());
        HttpEntity httpEntity = new StringEntity(message, ContentType.TEXT_PLAIN);
        return new RespStub(500, httpEntity);
    }
}